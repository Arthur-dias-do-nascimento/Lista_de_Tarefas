...
function RemoveTaskFromLocalStorage(id) {
    ...
    if (storedTasks) {
        let updatedTasks = storedTasks.filter(item => {
            return item.id !== id;
        });
...
function Edit(id) {
    ...
    `
        ...
        EditTaskFromLocalStorage(id, new_task)
        EditTaskList(id, new_task)
    }
}

function EditTaskFromLocalStorage(id, task) {
    let get_list = JSON.parse(localStorage.getItem('task'))

    for (let i in get_list) {
        if (get_list[i].id === id) {
            get_list[i].task = task
            localStorage.setItem('task', JSON.stringify(get_list))
        }
    }
}

function EditTaskList(id, task) {
